"""Minimal usage example for Ki67Segmenter (OpenVINO FP32 backend).

Identical CLI to the INT8 ONNX package's example.py — only the loaded model
changes. Load any RGB image, give a few click coordinates, get the predicted
masks and a visualization PNG.

Usage:
    python example.py image.jpg --points "120,80;200,150"
        → saves image.overlay.png next to the input

Multiple clicks per cell (';' within an instance, '|' between):
...
ENCODER = HERE / "encoder.xml"
DECODER = HERE / "decoder.xml"
